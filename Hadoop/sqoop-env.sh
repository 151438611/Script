...
--delete-target-dir -m 1 --fields-terminated-by "\t" --hive-import --hive-overwrite --create-hive-table --hive-database default --hive-table htb1 

# 从Mysql中导出db1.tb1表到HBase中;需要提前创建HBase数据表和列族
hbase(main):001:0> create 'hbtb1', { NAME => 'f1', VERSIONS => 5}
sqoop import --connect jdbc:mysql://master:3306/db1 --username root --password root --table tb1 --hbase-table hbtb1 --column-family f1 --hbase-row-key id --hbase-create-table -m 1
