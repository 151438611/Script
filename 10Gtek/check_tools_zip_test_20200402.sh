...
older_info() {
	...
	if [ -z "$(echo "$older_type" | grep -i cm)" ]; then
		if [ -n "$(echo $older_type | grep -i 10sfp)" -a -n "$(echo $older_remark | grep -Ei "hpp|aruba")" ]; then
			older_length=0
		else
	...
	*)
		if [ -n "$(echo $older_type | grep -Ei "10gsfp|xfp-xfp|0sfp")" ]; then older_num=$(($older_num_old * 3 + 1))
			...
			if [ -n "$(echo $older_remark | grep -i mcu)" ]; then
				[ -n "$(echo $older_remark | grep -i Encryption_bottom)" ] && older_num=$(($older_num_old * 5 + 9)) || older_num=$(($older_num_old * 5 + 5))
			else
...
code_info() {
	...
	if [ -n "$code_file" ] ; then
		...
		if [ -n "$(echo $older_all | grep -Ei "10gsfp|0sfp" | grep -i mcu)" ] ; then
...
do
	...
	if [ $(cat $input_txt | awk '{print $3}' | grep -c $scdh 2>/dev/null) -eq 1 ]; then
		...
		if [ -d $older_id ]; then
			older_info
			if [ -n "$(echo $older_type | grep -Ei "10gsfp|0sfp|xfp-xfp|zsp-zsp")" ]; then 
				# 10G-SFP ZSP的放码模板结构为：Port2/A0 Port5/A0
				port2=${older_id}/Port2/A0/
				port5=${older_id}/Port5/A0/
			...
			elif [ -n "$(echo $older_type | grep -Ei "q10-4s|qsfp-4sfp|qsfp-4xfp|zqp-4zsp")" ] ; then
				# 分支线缆的放码模板结构为：Port1/[A0|Page00|Page02] Port2/A0 ... Port5/A0
				port1=${older_id}/Port1/A0/
				[ ! -d $port1 ] && port1=${older_id}/Port1/Page00/
				port1_p02=${older_id}/Port1/Page02/
				port2=${older_id}/Port2/A0/
