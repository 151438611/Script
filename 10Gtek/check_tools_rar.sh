...
#获取编码完后的编码压缩rar文件
input_rar() {
  input_rar=$(ls -t *.rar | head -n1)
#判断是否存在未删除的解压出来的文件夹，删除了再解压刚传入的rar文件,-iname表示忽略大小写
  ls | grep -qi "wo" && find . -type d -iname "WO*" -exec rm -rf {} \; 2> /dev/null
  [ -z "$input_rar" ] && echo -e "\n rar文件不存在，请重新检查！！！\n" && exit
  rar x "$input_rar" 1> /dev/null || exit
}
older_info() {
#提取邮件中某个生产订单一整行内容
...
#提取订单编码数量,示例：30
  ...
  case $older_kind in
    "H3C码")
       older_num=$((older_num_old+2)) ;;
    *)
       if [ -n "$(echo $older_type | grep -Ei "8644/qsfp|qsfp/4xfp")" ] ; then older_num=$(($older_num_old * 2))
       elif [ -n "$(echo $older_type | grep -Ei "10GSFP|sfp-sfp")" ] ; then older_num=$(($older_num_old * 3 + 1))
       elif [ -n "$(echo $older_type | grep -i "xfp/sfp")" ] ; then older_num=$(($older_num_old * 4 + 1))
...
#提取编码中的第1位，03表示SFP类型，06表示XFP类型, 0D表示40G-QSFP, 11表示100G-ZQP，0F表示8644
      code_type=$(hexdump -C $code_file | awk 'NR==1{print $2}')
      case $code_type in
        "03") code_type="SFP" ;;
        "06") code_type="XFP" ;;
        "0D"|"0d") code_type="Q10" ;;
...
#提取编码中的第1行第14位，0C表示千兆，63/67表示10G, FF表示25G, 3C表示6G
      code_speed=$(hexdump -C $code_file | awk 'NR==1{print $14}')
      case $code_speed in
...
#提取编码中的第2行第4位，表示线缆的长度
     code_length=$(hexdump -C $code_file | awk 'NR==2{print $4}') ; code_length=$(echo $((0x$code_length)))
#提取编码中的第6行日期
     code_time_line=$(hexdump -C $code_file | awk -F "|" 'NR==6{print $2}')
     code_time=${code_time_line:4:6}
   else
     code_type=$(hexdump -C $code_file | awk 'NR==9{print $2}')
     case $code_type in
	"03") code_type="SFP" ;;
	"06") code_type="XFP" ;;
	"0D"|"0d") code_type="Q10" ;;
...
#提取编码中的第1行第14位，0C表示千兆，63/67表示10G, FF表示25G, 3C表示6G
     code_speed=$(hexdump -C $code_file | awk 'NR==9{print $14}')
     case $code_speed in
	"0c"|"0C") code_speed="1000BASE" ;;
	"3c"|"3C") code_speed="6G" ;;
	"63"|"64"|"67") code_speed="10G" ;;
...
#提取编码中的第2行第4位，表示线缆的长度
      code_length=$(hexdump -C $code_file | awk 'NR==10{print $4}')
#提取编码中的第6行日期
      code_time_line=$(hexdump -C $code_file | awk -F "|" 'NR==14{print $2}')
      code_time=${code_time_line:4:6}
      fi
      [ "$code_type" = "SFP" -a "$code_speed" = "25G" ] && code_type="ZSP"
...
check_end() {
#清除解压出来的编码文件夹，并重命名
  mv -f $input_rar old.rar &> /dev/null ; mv -f $input_txt old.txt &> /dev/null
...
#判断是否出现编码错误，出错就输出错误信息和编码中的十六进制文件。
  ...
  echo $(rar vb $input_rar | awk 'BEGIN {FS="/"}{print $1}' | sort -f | uniq) >> result
 fi
  echo -e "--------------------------------------------------------------------------------------\n" >> result
done
...
#显示编码压缩文件中的目录内容
       echo $(rar vb $input_rar | awk -F"/" '{print $1}' | sort -u)
    fi
...
for sn in $snall
do sn_start=$(echo $sn | awk -F"-" '{print $1}')
