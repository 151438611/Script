...
#    新增 QSFP_MCU、QSFP_MCU/2SFP_EEPROM 、ZQP_MCU/2ZSP_EEPROM 线缆需加密底层的写码模板,需LMM/Page00/Page02中都放password.txt文件,
...
order_info() {
	...
	order_remark=$(echo $order_all | awk '{print $8$9$10$11$12$13$14$15}')
	...
	*)
		...
		elif [ -n "$(echo $order_type | grep -i "zqp-zqp")" ]; then order_num=$(($order_num_old * 5 + 5))
	...
	elif [ -n "$(echo $order_remark | grep -i juniper)" -a $order_num_old -ge 50 ]; then order_kind=Juniper
	elif [ -n "$(echo $order_remark | grep -i arista)" -a $order_num_old -ge 50 ]; then
		[ -n "$(echo $order_type | grep -Ei "qsfp|q10|zsp")" ] && order_kind=Arista || order_kind=OEM
	elif [ -n "$(echo $order_remark | grep -i alcatel)" -a $order_num_old -ge 50 ]; then order_kind="Alcatel-lucent"
	elif [ -n "$(echo $order_remark | grep -i brocade)" -a $order_num_old -ge 50 ]; then order_kind=Brocade
	elif [ -n "$(echo $order_remark | grep -Ei "dell|force")" -a $order_num_old -ge 50 ]; then
		[ -n "$(echo $order_type | grep -i 10gsfp)" ] && order_kind=Dell || order_kind=OEM
	elif [ -n "$(echo $order_remark | grep -i "mellanox")" -a $order_num_old -ge 50 ]; then
		[ -n "$(echo $order_type | grep -i zqp)" ] && order_kind=Mellanox || order_kind=OEM
	elif [ -n "$(echo $order_remark | grep -Ei "huawei|intel|extreme")" -a $order_num_old -ge 50 ]; then order_kind=OEM
	# 非以上备注默认思科码代替
