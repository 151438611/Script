...
fun_get_net_hardware_2() {
	...
	if [ $net_hardware_num -gt 1 -a $net_hardware_num -le 5 ]; then 
		...
		elif [ -n "$(echo "$net_hardware" | grep -i X722)" ]; then
			nic_port_num=$(echo "$net_hardware" | grep -ci X722)
			if [ $nic_port_num -eq 2 ]; then nic_model="X722-10G-2S"
	...
	*)
		...
		case ${port_id_tmp:=1} in 
			...
			a|all)
				...
				do
					# 使用 { } & 将任务放入后台,多端口同时测试eth1 eth2 ... 
					# 若需要多端口依次按顺序来单个测试,则删除 { } 后的 & 号即可; 程序会依次测试完eth1,再启动测试eth2 ...
					{ 
